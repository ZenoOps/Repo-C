...
from agentric.lib.otel import configure_instrumentation
...
cors = CORSConfig(allow_origins=cast("list[str]", settings.app.ALLOWED_CORS_ORIGINS), allow_credentials=True, allow_methods=["*"])
alchemy = SQLAlchemyAsyncConfig(
    engine_instance=settings.db.get_engine(),
    before_send_handler="autocommit",
    session_config=AsyncSessionConfig(expire_on_commit=False,autoflush=True),
...
templates = TemplateConfig(
    engine=JinjaTemplateEngine(directory=settings.vite.TEMPLATE_DIR)
)
...
saq = SAQConfig(
    ...
    queue_configs=[
        QueueConfig(
            ...
            name="agentric_submissions",
            tasks=[
                "agentric.domain.requests.tasks.process_travelguard_claims",
                "agentric.domain.requests.tasks.process_covermore_claims",
            ],
...
open_telemetry_config = configure_instrumentation()
