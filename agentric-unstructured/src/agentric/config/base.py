...
class ViteSettings:
    ...
    USE_SERVER_LIFESPAN: bool = field(
        default_factory=get_env("VITE_USE_SERVER_LIFESPAN", True)
    )
    ...
    ENABLE_REACT_HELPERS: bool = field(
        default_factory=get_env("VITE_ENABLE_REACT_HELPERS", True)
    )
    """Enable React support in HMR."""
    BUNDLE_DIR: Path = field(
        default_factory=get_env(
            "VITE_BUNDLE_DIR", Path(f"{BASE_DIR}/domain/web/public")
        )
    )
    """Bundle directory"""
    RESOURCE_DIR: Path = field(
        default_factory=get_env("VITE_RESOURCE_DIR", Path("resources"))
    )
    """Resource directory"""
    TEMPLATE_DIR: Path = field(
        default_factory=get_env(
            "VITE_TEMPLATE_DIR", Path(f"{BASE_DIR}/domain/web/templates")
        )
    )
    """Template directory."""
...
class SaqSettings:
    ...
    USE_SERVER_LIFESPAN: bool = field(
        default_factory=get_env("SAQ_USE_SERVER_LIFESPAN", True)
    )
...
@dataclass
class OTEL:
    """Open Telemetry + Jaeger configurations."""

    EXPORTER_OTLP_ENDPOINT: str = field(
        default_factory=get_env("OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4317")
    )
    """The endpoint for the OTLP exporter to send telemetry data to."""
    SERVICE_NAME: str = field(default_factory=get_env("OTEL_SERVICE_NAME", "Claims"))
    """Name of the service generating the telemetry data, used to identify traces and metrics."""
    INSECURE: bool = field(default_factory=get_env("OTEL_EXPORTER_INSECURE", True))
    """Whether to use secure communication (e.g. HTTPS) when sending data to the OTLP endpoint."""
...
class LogSettings:
    ...
    SAQ_LEVEL: int = field(default_factory=get_env("SAQ_LOG_LEVEL", 10))
...
class MinioSettings:
    STORAGE_LOCATION: str = field(
        default_factory=get_env("MINIO_STORAGE_LOCATION", "./data")
    )
    PORT: str = field(default_factory=get_env("MINIO_PORT", "9000"))
    ROOT_USER: str = field(default_factory=get_env("MINIO_ROOT_USER", "admin"))
    ROOT_PASSWORD: str = field(
        default_factory=get_env("MINIO_ROOT_PASSWORD", "password")
    )
    BUCKET: str = field(default_factory=get_env("MINIO_BUCKET", "swparse"))
    ENDPOINT_URL: str = field(
        default_factory=get_env("MINIO_ENDPOINT", "http://minio:9000/")
    )
    JOB_FOLDER: str = field(default_factory=get_env("MINIO_JOB_FOLDER", "JOB"))
    PUBLIC_ENDPOINT_URL: str = field(
        default_factory=get_env("MINIO_PUBLIC_ENDPOINT", "http://localhost:9100/")
    )
    ...
    def get_client(self) -> S3FileSystem:
        s3 = S3FileSystem(
        ...
        if not s3.exists(self.BUCKET):
            s3.mkdir(self.BUCKET)
        return s3
...
class AppSettings:
    ...
    APP_LOC: str = "agentric.asgi:agentric"
    ...
    NAME: str = field(default_factory=lambda: "agentric")
    """Application name."""
    ...
    MODEL_NAME: str = field(default_factory=get_env("MODEL_NAME", ""))
    MODEL_FAIMLY: str = field(default_factory=get_env("MODEL_FAMILY", ""))
    ...
    USER_AGENT: str = field(default_factory=get_env("USER_AGENT", ""))
    TEAM_STATE_FOLDER_PATH: str = field(
        default_factory=get_env("TEAM_STATE_FOLDER_PATH", "")
    )
    AGENT_API_KEY: str = field(default_factory=get_env("AGENT_API_KEY", ""))

    AWS_SECRET_KEY: str = field(default_factory=get_env("AWS_SECRET_KEY", ""))
    AWS_ACCESS_KEY: str = field(default_factory=get_env("AWS_ACCESS_KEY", ""))
    AWS_REGION: str = field(default_factory=get_env("AWS_REGION", ""))

    ADMIN_ACCOUNTS: list[str] | str = field(default_factory=get_env("ADMIN_ACCOUNTS", [""]))
    ADMIN_PASSWORDS: list[str] | str = field(default_factory=get_env("ADMIN_PASSWORDS", [""]))

    USER_ACCOUNTS: list[str] | str = field(default_factory=get_env("USER_ACCOUNTS", ["*"]))
    USER_PASWDS: list[str] | str = field(default_factory=get_env("USER_PASSWORDS", [""]))

    TRAVEL_GUARD_ACCOUNTS: list[str] | str = field(default_factory=get_env("TRAVEL_GUARD_ACCOUNTS", [""]))
    TRAVEL_GUARD_PASSWORDS: list[str] | str = field(default_factory=get_env("TRAVEL_GUARD_PASSWORDS", [""]))

    COVER_MORE_ACCOUNTS: list[str] | str = field(default_factory=get_env("COVER_MORE_ACCOUNTS", [""]))
    COVER_MORE_PASSWORDS: list[str] | str = field(default_factory=get_env("COVER_MORE_PASSWORDS", [""]))
            ...
            if isinstance(value, str):
                ...
                if value.strip().startswith("[") and value.strip().endswith("]"):
                    try:
                        ...
                        raise ValueError(f"{field_name} is not a valid JSON list")
                else:
...
class Settings:
    ...
    stats: SaqSettings = field(default_factory=SaqSettings)
